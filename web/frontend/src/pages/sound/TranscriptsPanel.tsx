...
export function TranscriptsPanel() {
  ...
  const { data: speakersData } = useQuery({
    queryKey: ["audioSpeakers", "all"],
    queryFn: () => audioApi.speakers({ limit: 200 }).then((r) => r.data),
  });

  const { data, isLoading } = useQuery({
    queryKey: ["audioSegments", submittedQ, speakerId, timeFrom, timeTo],
    queryFn: () =>
      audioApi.segments({
  ...
  const reassignMutation = useMutation({
    mutationFn: ({ id, target }: { id: number; target: number | null }) =>
      audioApi.updateSegment(id, { speaker_id: target }),
    onSuccess: () => {
  ...
  const editMutation = useMutation({
    mutationFn: ({ id, text }: { id: number; text: string }) =>
      audioApi.updateSegment(id, { transcript: text }),
    onSuccess: () => {
  ...
  const deleteMutation = useMutation({
    mutationFn: (id: number) => audioApi.deleteSegment(id),
