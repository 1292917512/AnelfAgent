...
鉴权：Authorization 头携带 audiosync_openlist_token。
"""
...
def _endpoint() -> str:
    endpoint = str(get_config("audiosync_openlist_endpoint", "") or "").strip().rstrip("/")
    if not endpoint:
        raise OpenListError("未配置 OpenList 地址（audiosync_openlist_endpoint）")
    return endpoint


def _headers() -> Dict[str, str]:
    token = str(get_config("audiosync_openlist_token", "") or "").strip()
    return {"Authorization": token} if token else {}


def is_configured() -> bool:
    """OpenList 访问是否已配置。"""
    return bool(str(get_config("audiosync_openlist_endpoint", "") or "").strip())


def configured_root() -> str:
    """配置的 OpenList 监听根路径。"""
    return str(get_config("audiosync_openlist_path", "/") or "/").strip() or "/"
...
async def download(remote_path: str, dest_dir: Optional[str] = None) -> str:
    ...
    async with httpx.AsyncClient(timeout=300.0, follow_redirects=True) as client:
        ...
        fd, local_path = tempfile.mkstemp(
            prefix="audiosync_dl_", suffix=suffix, dir=dest_dir)
