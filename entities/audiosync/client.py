...
async def transcribe(audio_path: str, source_time: str = "") -> List[Dict[str, Any]]:
    ...
    try:
        try:
            ...
            log(f"音频预处理跳过（原始文件直传）: {exc}", "DEBUG", tag="音源同步")
    ...
    log(f"FunASR 转写完成: {os.path.basename(audio_path)} → {len(segments)} 段", tag="音源同步")
    return segments
...
async def _post_with_retry(
    ...
    for attempt in range(2):
        try:
            ...
            # 服务重启/瞬时网络故障：重试一次
            if attempt == 0:
                log(f"FunASR 连接失败，1s 后重试: {exc}", "WARNING", tag="音源同步")
