...
export function RecordingsPanel() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ["audiosyncRecordings"],
    queryFn: () => audioApi.recordings({ limit: 100 }).then((r) => r.data),
    refetchInterval: 15_000,
  });

  const invalidate = () => {
    queryClient.invalidateQueries({ queryKey: ["audiosyncRecordings"] });
    queryClient.invalidateQueries({ queryKey: ["audiosyncSegments"] });
  ...
  const deleteMutation = useMutation({
    mutationFn: (path: string) => audioApi.deleteRecording(path),
    onSuccess: () => {
  ...
  const rebuildMutation = useMutation({
    mutationFn: (path: string) => audiosyncApi.rebuildRecordings([path]),
    onSuccess: (r) => {
...
function RecordingCard({
  ...
  const { data: segments, isLoading } = useQuery({
    queryKey: ["audiosyncRecordingSegments", recording.path],
    queryFn: () =>
      audioApi.segments({ recording_path: recording.path, limit: 100 })
        .then((r) => r.data),
    enabled: expanded,
  });
