...
export function OverviewPanel() {
  ...
  const { data: stats } = useQuery({
    queryKey: ["audiosyncLibraryStats"],
    queryFn: () => audioApi.stats().then((r) => r.data),
    refetchInterval: 10_000,
  });

  const { data: watch } = useQuery({
    queryKey: ["audiosyncSyncStatus"],
    queryFn: () => audiosyncApi.syncStatus().then((r) => r.data),
    refetchInterval: 10_000,
  });

  const { data: config } = useQuery({
    queryKey: ["audiosyncConfig"],
    queryFn: () => audiosyncApi.config().then((r) => r.data),
    refetchInterval: 30_000,
  });

  const { data: preview, isLoading: previewLoading } = useQuery({
    queryKey: ["audiosyncSyncPreview"],
    queryFn: () => audiosyncApi.syncPreview().then((r) => r.data),
    refetchInterval: 15_000,
  });

  const { data: sourceStatus } = useQuery({
    queryKey: ["audiosyncSourceStatus"],
    queryFn: () => audiosyncApi.sourceStatus().then((r) => r.data),
    refetchInterval: 30_000,
  });

  const invalidateAll = () => {
    queryClient.invalidateQueries({ queryKey: ["audiosyncLibraryStats"] });
    queryClient.invalidateQueries({ queryKey: ["audiosyncSyncStatus"] });
    queryClient.invalidateQueries({ queryKey: ["audiosyncSyncPreview"] });
    queryClient.invalidateQueries({ queryKey: ["audiosyncRecordings"] });
  };
  ...
  const pauseMutation = useMutation({
    mutationFn: (paused: boolean) =>
      audiosyncApi.updateConfig({ audiosync_watch_paused: paused }),
    onSuccess: (_r, paused) => {
      toast.success(paused ? t("messages.syncPaused") : t("messages.syncResumed"));
      queryClient.invalidateQueries({ queryKey: ["audiosyncSyncStatus"] });
      queryClient.invalidateQueries({ queryKey: ["audiosyncConfig"] });
    },
    onError: () => toast.error(t("messages.opFailed")),
  });

  const ingestToken = config?.items.find((i) => i.key === "audiosync_ingest_token");
  const ingestEnabled = Boolean(String(ingestToken?.current_value ?? "").trim());
  ...
  const checks = [
    ...
    },
    {
      ...
      hint: `vec ${stats?.vec_available ? "✓" : "✗"} · fts ${stats?.fts_available ? "✓" : "✗"}`,
    },
  ];
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        {sourceStatus?.configured && (
          <div className="flex items-center gap-1.5 rounded-full border border-border bg-card px-3 py-1.5 text-xs">
            ...
            <span className="text-foreground">{t("overview.source")}</span>
            <span className={sourceStatus.reachable ? "text-muted" : "text-danger"}>
              {sourceStatus.reachable ? `${sourceStatus.latency_ms}ms` : sourceStatus.error}
            </span>
          </div>
        )}
      </div>
      ...
      <Card title={t("overview.mirrorTitle")}>
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_auto_1fr] gap-4 items-stretch">
          ...
          <div className="rounded-lg border border-border bg-elevated p-3 space-y-2">
            ...
            {sourceStatus?.configured && !sourceStatus.reachable && (
              <p className="text-xs text-danger">{sourceStatus.error}</p>
            )}
              ...
              <>
                <div className="flex items-center gap-1">
                  <Button
                    ...
                    disabled={!stats?.asr_configured || watch?.paused}
