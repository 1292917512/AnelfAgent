...
```bash
git clone https://github.com/yourname/AnelfAgent.git
cd AnelfAgent
...
└── scripts/                  # 工具脚本
    ├── secrets-remove.sh     #   脱敏（移除敏感配置）
    └── secrets-restore.sh    #   恢复（还原敏感配置）
```
...
项目提供完整的脱敏工具链，个人配置与框架代码完全分离：

```bash
# 提交前：移除敏感文件到 .secrets/
./scripts/secrets-remove.sh

# 开发时：从 .secrets/ 恢复
./scripts/secrets-restore.sh
```

敏感文件（API Key、Token、记忆数据等）已通过 `.gitignore` 排除，仓库中仅保留 `*.example.json` 模板。
