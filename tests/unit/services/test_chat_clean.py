"""聊天历史展示清洗（services.chat.clean_message_for_display）单元测试。
...
class TestCleanMessageKind:
    def test_tool_summary_kind_detected(self) -> None:
        result = clean_message_for_display({"role": "system", "content": _SUMMARY})
    ...
    def test_tool_summary_structured_entries(self) -> None:
        """tool_summary 消息附结构化条目：前端不再解析 prose。"""
        result = clean_message_for_display({"role": "system", "content": _SUMMARY})
        summary = result["summary"]
        assert summary["count"] == 3
        calls = [e["call"] for e in summary["entries"]]
        assert any(c.startswith("memorize(") for c in calls)
        assert any(c.startswith("send_message(") for c in calls)
        assert any(c.startswith("end_reply(") for c in calls)
        memorize = next(e for e in summary["entries"] if e["call"].startswith("memorize("))
        # 参数正文自带冒号与 JSON 数组，result 仍取行末 → 之后的内容
        assert memorize["result"].startswith('{"ok": true')
        send = next(e for e in summary["entries"] if e["call"].startswith("send_message("))
        assert send["result"] == "已发送 -> web_user"

    def test_system_notice_kind(self) -> None:
        result = clean_message_for_display({"role": "system", "content": "[系统] 计划已完成"})
        assert result["kind"] == "system_notice"

    def test_execution_steps_kind(self) -> None:
        result = clean_message_for_display({"role": "system", "content": "[执行步骤]\n1. 检索记忆"})
        assert result["kind"] == "system_notice"

    def test_plain_message_no_kind(self) -> None:
        result = clean_message_for_display({"role": "assistant", "content": "普通回复"})
...
class TestCleanMessageTs:
    def test_ts_ns_seconds_to_epoch(self) -> None:
        result = clean_message_for_display({"role": "user", "content": "hi", "ts_ns": 1785900000})
        assert result["ts"] == 1785900000
        assert result["timestamp"]

    def test_ts_ns_nanoseconds_to_epoch(self) -> None:
        result = clean_message_for_display({"role": "user", "content": "hi", "ts_ns": 1785900000_000_000_000})
        assert result["ts"] == 1785900000

    def test_missing_ts_ns_no_ts(self) -> None:
        result = clean_message_for_display({"role": "user", "content": "hi"})
