...
from agent.mind.tools import think_loop as tl  # noqa: E402
from agent.mind.tools.think_loop import ThinkMode, think_loop  # noqa: E402
...
def _tool_round_result(name: str) -> SimpleNamespace:
    return SimpleNamespace(
        ...
        tool_calls=[SimpleNamespace(
            id=f"tc_{name}", name=name, arguments="{}",
...
def _text_round_result(text: str) -> SimpleNamespace:
    return SimpleNamespace(
...
class _FakeMind:
    """最小 Mind 替身：第 1 轮返回工具调用，第 2 轮返回纯文本结束。"""

    def __init__(self) -> None:
        self.pfc = _FakePfc()
        self.compressor = None
        self._rounds: List[SimpleNamespace] = [
            _tool_round_result("recall"),
            _text_round_result("想起来了～"),
        ]
...
class TestThinkLoopChainBreakpoint:
    ...
    @pytest.fixture
    def deliver_mock(self, monkeypatch):
        mock = AsyncMock(return_value=True)
        monkeypatch.setattr(tl, "deliver_text", mock)
        return mock

    async def test_think_loop_never_marks_messages(
        self, anything, deliver_mock,
    ) -> None:
