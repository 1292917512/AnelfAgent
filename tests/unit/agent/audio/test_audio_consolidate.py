...
from agent.audio.consolidate import consolidate, find_merge_clusters
...
async def _mk_pending(store: AudioStore, vector: list[float], audio_ms: int) -> dict:
...
class TestClusters:
    async def test_similar_pending_clustered(self, store: AudioStore) -> None:
    ...
    async def test_below_threshold_not_clustered(self, store: AudioStore) -> None:
        await _mk_pending(store, vec(0), 1000)
        await _mk_pending(store, vec(1), 1000)
        assert await find_merge_clusters(store) == []

    async def test_dry_run_then_execute(self, store: AudioStore) -> None:
        a = await _mk_pending(store, vec(0), 5000)
    ...
    async def test_confirmed_excluded_by_default(self, store: AudioStore) -> None:
