...
class TestMatchAndIdentify:
    async def test_enroll_and_match_known(self, store: AudioStore) -> None:
    ...
    async def test_identify_unknown_creates_pending(self, store: AudioStore) -> None:
    ...
    async def test_per_speaker_threshold_overrides_global(self, store: AudioStore) -> None:
        s = await matcher.enroll(store, "张三", vec(0))
    ...
    async def test_match_topk_candidates(self, store: AudioStore) -> None:
        await matcher.enroll(store, "张三", vec(0))
...
class TestConfirmAndMerge:
    ...
    async def test_merge_fuses_pool_and_segments(self, store: AudioStore) -> None:
    ...
    async def test_merge_same_id_rejected(self, store: AudioStore) -> None:
        s = await matcher.enroll(store, "张三", vec(0))
...
class TestIngest:
    async def test_ingest_known_and_new(self, store: AudioStore) -> None:
        s = await matcher.enroll(store, "张三", vec(0))
        ...
        # 默认挂接（audio_attach_unidentified）：无声纹段归入前一段的说话人
    ...
    async def test_ingest_empty_payload(self, store: AudioStore) -> None:
        result = await ingest_payload(IngestPayload(), store=store)
        assert result.ingested == 0
        assert result.results == []

    async def test_abs_time_overrides_base_offset(self, store: AudioStore) -> None:
...
class TestIngestQualityGate:
    async def test_noise_segments_skipped(self, store: AudioStore) -> None:
        """纯标点/空白段：跳过不建档不计片段。"""
        from agent.audio.ingest import is_noise_text
        assert is_noise_text("，") and is_noise_text("。 ")
        assert is_noise_text("...?!") and is_noise_text("  ")
        assert not is_noise_text("你好") and not is_noise_text("嗯。")
    ...
    async def test_short_segments_not_enrolled(self, store: AudioStore) -> None:
        """短于 min_audio_ms 的段：不匹配不建档（防过度分裂）；
        关闭挂接配置时归属为未知。"""
        from core.config import ConfigManager
        ConfigManager.set("audio_attach_unidentified", False)
        result = await ingest_payload(IngestPayload(
            source_file="/nas/a.wav",
            segments=[
    ...
    async def test_attach_unidentified_default_on(self, store: AudioStore) -> None:
...
class TestPrunePending:
    async def test_prune_only_empty_pending(self, store: AudioStore) -> None:
    ...
    async def test_prune_all_pending_with_samples(self, store: AudioStore) -> None:
