...
class TestPFCFiltering:
    def test_disabled_common_tool_filtered(self, channel_manager) -> None:
        ...
        assembly = ToolAssembly(channel_manager=cm)

        names_fake = {s["function"]["name"] for s in assembly.get_channel_tool_schemas("fake")}
        names_fake2 = {s["function"]["name"] for s in assembly.get_channel_tool_schemas("fake2")}
