...
class TestScopeSuffix:
    ...
    def test_parse_entity_scope(self) -> None:
        ...
        assert parse_entity_scope("user_webui:web_user#abc") == ("user", "webui", "web_user", "abc")
        assert parse_entity_scope("") == ("", "", "", "")
        assert parse_entity_scope("invalid") == ("", "", "", "")

    def test_adapter_scope_format(self) -> None:
        """带 adapter_key 的消息产生含频道前缀的 scope（跨频道同号隔离）。"""
        msg = MessageUser(uid="123", adapter_key="qq", text_content="hi")
        assert msg.scope_id == "qq:123"
        assert msg.entity_scope == "user_qq:123"
        web = MessageUser(uid="web_user", adapter_key="webui", session_id="a", text_content="hi")
        assert web.entity_scope == "user_webui:web_user#a"
...
class TestPfcScopeQueues:
    ...
    async def test_consume_scope_clears_state(self) -> None:
        ...
        assert pfc.consume_scope_task("user_webui:web_user#a") is True
        assert pfc.get_unread_count("user_webui:web_user#a") == 0
...
class TestResolveReplyTarget:
    async def test_session_propagates_to_reply_message(self) -> None:
        ...
        msg = resolve_reply_target(mind, "user_webui:web_user#tab1")
        assert isinstance(msg, MessageAssistant)
        assert msg.session_id == "tab1"
        assert msg.adapter_key == "webui"
        assert msg.entity_scope == "user_webui:web_user#tab1"
        # 消费后队列清空
        assert pfc.peek_all_tasks() == []
...
class TestSwitchSession:
    async def test_switch_enqueues_target_scope(self) -> None:
        ...
        try:
            ...
            assert result["scope"] == "user_webui:web_user#tab9"
    ...
    async def test_list_sessions_reports_unread(self) -> None:
        ...
        try:
            ...
            assert entry["scope"] == "user_webui:web_user#tab1"
