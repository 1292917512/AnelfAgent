...
class TestPromptCacheManager:
    ...
    def test_scope_shared_content_addressed(self) -> None:
        """内容寻址：同 layer 同 hash 的条目跨 scope 共享（不重复构建/存储）。"""
        mgr = PromptCacheManager()
        calls = {"n": 0}
        ...
        mgr.get_or_build("s1", LAYER_STABLE, "h", builder)
        content, hit = mgr.get_or_build("s2", LAYER_STABLE, "h", builder)
        assert content == "shared" and hit
        assert calls["n"] == 1, "相同内容跨 scope 应命中同一条目"
