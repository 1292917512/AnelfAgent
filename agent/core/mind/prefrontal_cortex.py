...
from agent.core.messages import Everything, EverythingGroup, EntityData
from agent.core.mind.autonomous import MindTask, TaskType
from agent.core.utils.unique_queue import UniqueQueue
from agent.core.storage.data_center import EverythingData
...
if TYPE_CHECKING:
    ...
    from agent.core.storage.data_center import ConversationData
...
_TOOL_USAGE_RULES = (
    ...
    "3. 工具的结果会直接出现再上下文中是以user用户的形式，但执行的结果是自己执行的结果。\n"
...
_ERROR_RETRY_WARNING = (
    ...
    "应尝试不同参数、换用其他工具，或直接调用 end_reply 结束。"
)
