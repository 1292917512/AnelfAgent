...
audio_segments.transcript 的文本向量由记忆子系统的 EmbeddingWorker 统一消化：
...
def register_backlog() -> None:
    """注册片段向量回填 backlog（bootstrap 导入本模块时调用一次）。"""
    try:
        from agent.memory.embedding import register_embedding_backlog
        register_embedding_backlog("audio_segments", _segments_backfill)
    except Exception as exc:
        log(f"音频片段 embedding backlog 注册失败: {exc}", "DEBUG", tag=_LOG_TAG)
