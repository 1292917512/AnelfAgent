...
- 沿清单累计时长找到覆盖该区间的源文件（经 source_fetch 统一取回：
  本地直读 / 组件远程下载），单文件直切、跨文件先并后切，
  得到与入库时完全一致的音频切片
"""
...
from . import ffmpeg, matcher, source_fetch
from .providers import KIND_ASR, get_audio_registry
...
def _clips_dir() -> str:
    path = os.path.join(workspace_root(), "audio_clips")
...
async def build_clip(
    ...
    try:
        ...
        for item in overlapping:
            try:
                local, is_temp = await source_fetch.fetch_source(item["path"])
        ...
        fd, clip_path = tempfile.mkstemp(prefix="audio_clip_", suffix=".wav")
        os.close(fd)
...
async def listen_segment(
    ...
    *,
    ...
    if await get_audio_registry().resolve(KIND_ASR) is None:
        raise ListenError("无可用 ASR 提供者（配置转写服务后可用）")
    ...
    if not recording_path:
        raise ListenError("该片段无录制单元归属（非同步入库），无法回听")
    ...
    try:
        ...
        try:
            fresh_segments = await get_audio_service().transcribe(
                clip_path, source_time=source_time)
        except Exception:
            # 转写失败：切片已无消费方，一并清理（防临时文件泄漏）
            if clip_path in temps:
                try:
                    os.unlink(clip_path)
                except OSError:
                    pass
    ...
    if apply and (fresh_text or fresh_vector):
        ...
        if fresh_speaker and fresh_speaker.get("matched") \
            ...
            result["applied"] = True
        if result["applied"]:
            log(f"回听订正片段 {segment_id}: {fresh_text[:40]}", tag=_LOG_TAG)
