...
def ffmpeg_bin() -> str:
    """ffmpeg 可执行文件路径（audio_ffmpeg_bin 配置）。"""
    return str(get_config("audio_ffmpeg_bin", "ffmpeg") or "ffmpeg").strip() or "ffmpeg"
...
def _ffprobe_bin() -> str:
    configured = str(get_config("audio_ffmpeg_bin", "ffmpeg") or "").strip()
...
async def _run(cmd: list[str]) -> tuple[int, bytes]:
    ...
    try:
        ...
        raise PreprocessError(
            f"找不到可执行文件: {cmd[0]}（请安装 ffmpeg 或配置 audio_ffmpeg_bin）"
        ) from exc
...
async def mean_volume_db(src_path: str) -> Optional[float]:
    ...
    try:
        ...
        raise PreprocessError(
            f"找不到可执行文件: {_ffmpeg_bin()}（请安装 ffmpeg 或配置 audio_ffmpeg_bin）"
        ) from exc
...
async def ensure_16k_mono_wav(src_path: str) -> tuple[str, bool]:
    ...
    fd, tmp_path = tempfile.mkstemp(prefix="audio_", suffix=".wav")
    os.close(fd)
    try:
        await _run([
            _ffmpeg_bin(), "-y", "-v", "error",
        ...
        try:
            ...
            pass
        raise
    log(f"音频预处理: {os.path.basename(src_path)} → 16k 单声道 WAV", "DEBUG", tag="音频")
...
async def merge_to_wav(file_paths: list[str]) -> str:
    ...
    if len(ordered) == 1:
        ...
        fd, tmp_path = tempfile.mkstemp(prefix="audio_merge_", suffix=".wav")
        try:
    ...
    fd_list, list_path = tempfile.mkstemp(prefix="audio_concat_", suffix=".txt")
    fd_out, out_path = tempfile.mkstemp(prefix="audio_merge_", suffix=".wav")
    os.close(fd_out)
    try:
        ...
        try:
            ...
            pass
    log(f"音频合并: {len(ordered)} 个文件 → 16k 单声道 WAV", "DEBUG", tag="音频")
    return out_path
