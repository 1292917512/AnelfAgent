...
def ssrf_protection_enabled() -> bool:
    """SSRF 防护开关（retrieval_ssrf_protection，默认开）。"""
    try:
        return bool(ConfigManager.get("retrieval_ssrf_protection", True))
    except Exception:
        return True
...
def read_page(
    ...
    *,
    ...
    """
    if respect_robots:
        from agent.retrieval.robots import is_allowed
    ...
    from agent.retrieval.extractor import (
        _bs4_to_text,
