...
| 语音 AI 工具 | `agent/realtime/tools.py`（voice 组，bootstrap 激活） | `realtime_status`（会话/模式/配置/last_say_error）与 `realtime_say`（仲裁感知的主动语音：用户说话/她说话/思考中显式可重试拒绝；提交即异步播放，失败记 last_say_error 供复查） |
...
| 音源同步实体 | `entities/audiosync/` | 只做"音源 → 核心库"搬运：目录镜像同步（**来源组件化**——`framework.AudioSyncSource` 契约 + `sources/` 扫描注册，local_dir/OpenList 两组件，priority 链选取生效源）+ 上游推送（/api/entity/audiosync/ingest，audiosync_ingest_token 自校验 fail-closed）+ FunASR 组件注册 + 出站 webhook（outbound.py）；配置键 audiosync_*（entity/audiosync 组） |
