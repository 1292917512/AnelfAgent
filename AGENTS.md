...
**测试体系**：`tests/` 分两层——`tests/unit/`（纯 mock/纯函数/tmp_path，快速）与 `tests/integration/`（真实应用组装或需外部凭证，需凭证的用例 env-gated 自动 skip）；目录归属由根 `tests/conftest.py` 自动打 `unit`/`integration` marker，无需手写。根 conftest 全局隔离 ConfigManager（指向 tmp_path），新测试不得读写真实 `config/`。运行：`uv run pytest`（全量）/ `uv run pytest tests/unit`（快速）/ `uv run pytest -m integration`；加 `-n auto` 并行（已装 pytest-xdist，全量约 64s→26s，CI 已启用；单测调试/`--pdb` 时去掉）。CI（`.github/workflows/ci.yml`）在 push/PR 时执行 ruff + mypy（core 三平台必过、全量观察）+ pytest（并行含覆盖率）+ 前端 lint/build；经 `changes` job（dorny/paths-filter）路径感知——`**/*.py`/`pyproject.toml`/`uv.lock` 触发 python job，`web/frontend/**` 触发 frontend job，文档类提交全跳过，`workflow_dispatch` 手动触发全量；双 job 均有 timeout-minutes 挂起护栏，覆盖率产物失败也上传（保留 7 天）。
