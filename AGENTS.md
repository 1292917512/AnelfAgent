...
| 语音 AI 工具 | `agent/realtime/tools.py`（voice 组，bootstrap 激活） | `realtime_status`（会话/模式/配置/默认音色/last_say_error）；`realtime_reply`（**通话轮应答出口**：回复直接 TTS 说给通话另一头并 end_turn，通话中不用 send_message 发文字）；`realtime_say`（轮次外主动开口，忙时可重试拒绝）。两者均可指定 voice 音色（默认经 realtime_tts_voice，AI 可用 sound_config set realtime_voice 热改） |
...
| 通话应答纪律注入 | `agent/realtime/context.py`（RealtimeCallProvider，volatile 层） | 通话进行期间每轮注入一行提醒（应答用 realtime_reply 说出来、不发文字）；无通话零注入，realtime_context_inject 可关 |
